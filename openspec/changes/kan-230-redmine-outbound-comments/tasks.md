...
- [ ] 3.1 **Gate before code (JD-003):** Validate mutation lock order against `connection → binding → work`; inversion stops apply for design amendment/re-review.
- [ ] 3.2 **Gate before code (JD2-002):** Confirm `IntegrationSyncWork.refId` semantics. If it cannot store the parent issue ref, stop for design amendment/re-review.
