...
- [x] 2b.1 **[SCHEMA/MIGRATION]** `packages/api/prisma/schema.prisma` — remove `dueDate DateTime?` from `Issue` model. Migration `20260614201154_ppm_w1_pr2b_duedate_drop`: UPSERT backfill into issue_schedules FIRST, then `ALTER TABLE "issues" DROP COLUMN "due_date"`. Applied cleanly to kanon (dev) + kanon_test. `prisma generate`; `tsc --noEmit` clean.
- [x] 2b.2 `packages/api/src/modules/issue/service.ts:134` — HARD-REMOVED `dueDate` from `prisma.issue.create` data; no forwarding.
- [x] 2b.3 `packages/api/src/modules/issue/service.ts:453` — HARD-REMOVED `dueDate` write path from `updateIssue`; no forwarding alias.
- [x] 2b.4 `packages/api/src/modules/issue/schema.ts:24,42` — HARD-REMOVED `dueDate` from `CreateIssueBody` and `UpdateIssueBody` Zod schemas.
- [x] 2b.5 `packages/api/src/modules/issue/__tests__/service.test.ts` and `packages/shared/src/issue.real-contract.test.ts:60` — removed `dueDate: null` from fixture (no longer a Prisma field); dueDate-undefined assertion at line 149 kept (still valid contract doc). All 1292 tests pass.
- [x] 2b.6 `packages/mcp/src/types.ts:120,137` — HARD-REMOVED `dueDate` from `CreateIssueInput` and `UpdateIssueInput`.
- [x] 2b.7 `packages/mcp/src/tools/issues.ts:87,101,120,129` — removed all `dueDate` destructuring and body forwarding from create_issue and update_issue handlers.
- [x] 2b.8 `packages/mcp/src/transforms.ts:31` — removed `"dueDate"` from `ISSUE_LIST_FIELDS` allowlist.
- [x] 2b.9 `packages/mcp/src/transforms.test.ts:34` and `packages/mcp/src/tools/issues.test.ts:49` — removed `dueDate` from fixtures. `pnpm test` in packages/mcp: 316 passing, 0 failing.
- [x] 2b.10 `packages/web/src/features/board/use-create-issue-mutation.ts:20` — removed `dueDate?: string` from `CreateIssueInput` interface.
- [x] 2b.11 **[TEST]** `packages/api/src/modules/issue/__tests__/duedate-removal.integration.test.ts` — NEW: 3 integration tests proving: POST with dueDate body field succeeds (Zod strips unknown field) and response has no dueDate; GET /api/projects/:key/issues list items have no dueDate; GET /api/issues/:key detail has no dueDate. All pass.
