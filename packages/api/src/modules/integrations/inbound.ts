...
} from "../issue/transition-observation.js";
...
export async function claimBindingPollLease(
  ...
  return database.$transaction(async (transaction) => {
    ...
    if (!binding.bootstrapCutoff)
      throw new Error("Inbound binding cutoff disappeared during claim");
...
function outboundIssueIds(description: string | null): string[] {
  ...
  return [
    ...description.matchAll(/<!-- kanon-issue:([0-9a-f]{8}(?:-[0-9a-f]{4}){3}-[0-9a-f]{12}) -->/gi),
  ].map((match) => match[1]!);
}
...
async function recordInboundFailure(
  ...
  return database.$transaction(async (transaction) => {
    ...
    if (["applied", "conflict", "skipped"].includes(application.state) || activeClaim) {
  ...
  for (const change of comments) {
    ...
    if (change.operation !== "upsert" || !("body" in change.fields)) {
      ...
      await transaction.notification.deleteMany({
        where: {
          OR: [{ commentId: comment.id }, { mentionId: { in: mentions.map(({ id }) => id) } }],
        },
      });
      ...
      const localEvidence = {
        ...
        bodySha256,
        ...(uncertainWorkIds.length > 0 ? { outboundWriteUncertain: true, uncertainWorkIds } : {}),
      };
      const remoteEvidence = {
        provider: "redmine",
        remoteIssueId,
    ...
    const remoteAuthor = await transaction.integrationExternalIdentity.upsert({
      ...
      update: {
        ...(change.actor.username === undefined ? {} : { remoteLogin: change.actor.username }),
        remoteDisplayName: change.actor.displayName,
      },
    });
...
async function convergeLinkedIssue(
  ...
  const transitionObservationOccurrenceId = randomUUID();
  ...
  const outcome = await database.$transaction(
    async (transaction) => {
      ...
      for (const field of ISSUE_SYNC_FIELDS.filter((candidate) =>
        activeBlockedFields.has(candidate)
      )) {
        delete patch[field];
        ...
        conflicts[field] = {
          reason: ["missing-baseline", "diverged", "mapping", "invalid"].includes(String(reason))
      ...
      for (const field of ["title", "description", "priority", "assigneeId"] as const) {
        if (Object.prototype.hasOwnProperty.call(patch, field))
          issueData[field] = patch[field] as never;
      }
      if (Object.prototype.hasOwnProperty.call(patch, "state")) {
      ...
      let transitionObservationLaneKey: string | null = null;
      if (Object.prototype.hasOwnProperty.call(patch, "state")) {
        const observationContext = await resolveTransitionObservationContext(
          transaction,
          binding.actorMemberId
        );
        transitionObservationLaneKey = await enqueueTransitionObservationTx(transaction, {
          occurrenceId: transitionObservationOccurrenceId,
          issue: {
            id: issue.id,
            key: issue.key,
            from: issue.state,
      ...
      if (scheduleFields.length) {
        ...
        if (Object.prototype.hasOwnProperty.call(patch, "dueDate")) {
          scheduleData.dueDate =
            typeof patch.dueDate === "string" ? new Date(`${patch.dueDate}T00:00:00.000Z`) : null;
        }
        ...
        await transaction.issueSchedule.upsert({
          ...
          create: {
            issueId: issue.id,
            ...scheduleData,
          } as Prisma.IssueScheduleUncheckedCreateInput,
          update: scheduleData,
        });
      }
          ...
          if (fields && conflictFields.some((field) => Object.hasOwn(fields, field))) {
            const remainingFields = Object.fromEntries(
              Object.entries(fields).filter(
                ([field]) => !conflictFields.includes(field as IssueSyncField)
              )
            );
            await transaction.integrationSyncWork.update({
      ...
      const metadata = issueSyncMetadata(ref.metadata, {
        ...
        ...(remote.state !== undefined && !stateConflict
          ? {
              completedAt: remote.state === "done" ? (detail.closedAt ?? detail.changedAt) : null,
            }
          : {}),
        fields: nextBaseline,
      });
      ...
      return {
        ...
        transition: Object.prototype.hasOwnProperty.call(patch, "state")
          ? {
              ...
              from: issue.state,
              to: patch.state as IssueState,
              observationLaneKey: transitionObservationLaneKey!,
            }
          : null,
      };
    },
  ...
  await publishTransitionObservationLanes([transition.observationLaneKey]);
  ...
  for (const effect of effects) {
    if (effect.status === "rejected")
      console.error("Redmine transition side effect failed", effect.reason);
  }
...
async function retryBinding(
  ...
  if (connection.provider !== "redmine") {
    throw new AppError(
      ...
      "Connection is not a Redmine integration"
    );
  }
...
export async function retryRedmineIssueImport(
  ...
  try {
    ...
    if (detail.identity.remoteId !== claim.remoteId || detail.changedAt < claim.remoteUpdatedAt) {
      throw new AppError(
        409,
  ...
  if (!ref) {
    ...
    try {
      ...
      if (
        ...
        return recordInboundFailure(database, binding, change, null, error, detail.sourceVersion);
      }
      throw error;
    }
  }
...
async function pollBinding(
  ...
  dependencies: Required<
    Pick<InboundSyncDependencies, "decrypt" | "createSource" | "loadIssueDetail" | "logger" | "now">
  > & { leaseMs: number }
) {
