...
describe("Prisma audit census repository", () => {
  it("loads a durable partial run and atomically writes replay-safe observations with its checkpoint", async () => {
    ...
    const repository = createPrismaAuditCensusRepository(fake.database as never);
  ...
  it("resumes a prior matching scope, finalizes behind the same fence, and refuses stale ownership", async () => {
    ...
    expect(fake.runs.update).toHaveBeenLastCalledWith(expect.objectContaining({ data: expect.objectContaining({ state: "complete" }) }));
    ...
    await expect(createPrismaAuditCensusRepository(stale.database as never).commitIssue(lease, {
  ...
  it("keeps invalid or unbounded failure detail out of durable state", async () => {
    ...
    await expect(createPrismaAuditCensusRepository(fake.database as never).markFailed(lease, "provider response: secret body"))
      .resolves.toBe(true);
    ...
    await expect(createPrismaAuditCensusRepository(safe.database as never).markFailed(lease, "timeout")).resolves.toBe(true);
