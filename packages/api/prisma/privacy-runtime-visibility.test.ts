    ...
    it("hides held rows from runtime while preserving owner recovery visibility", async () => {
      ...
      expect(
        (
          await sql([
            "postgres",
            `SELECT title,description IS NULL,privacy_hold_generation,privacy_held_at IS NOT NULL FROM issues WHERE id='${issue}'; SELECT snapshot_schema,count(*) OVER() FROM privacy_quarantine.issue_content WHERE issue_id='${issue}'; SELECT count(*),count(DISTINCT store_kind) FROM privacy_authority.held_row_associations WHERE issue_id='${issue}'; SELECT count(*) FROM privacy_authority.held_row_associations WHERE store_kind='domain_event_outbox' AND row_pk='${related}'; SELECT metadata IS NULL FROM external_refs WHERE id='${related}'; SELECT payload='{}'::jsonb FROM integration_sync_work WHERE id='${related}'; SELECT outcome IS NULL FROM integration_inbound_applications WHERE id='${related}'; SELECT local_evidence='{}'::jsonb AND remote_evidence='{}'::jsonb FROM integration_conflicts WHERE id='${related}'; SELECT outcome IS NULL FROM integration_inbound_applications WHERE id='${parentOnlyApplication}'; SELECT local_evidence='{}'::jsonb AND remote_evidence='{}'::jsonb FROM integration_conflicts WHERE id='${parentOnlyConflict}'; SELECT outcome FROM integration_inbound_applications WHERE id='${competingParentApplication}'; SELECT local_evidence,remote_evidence FROM integration_conflicts WHERE id='${competingParentConflict}'; SELECT list_summary='{}'::jsonb FROM triage_proposals WHERE id='${related}'; SELECT count(*) FROM triage_proposal_contents WHERE id='${related}'; SELECT reason IS NULL AND details IS NULL FROM triage_proposal_lifecycle_events WHERE id='${related}'; SELECT count(*) FROM mcp_proposals WHERE id='${related}'; SELECT count(*) FROM mcp_proposals WHERE id='${unscoped}'; SELECT issue_key='[privacy hold]' AND reason IS NULL FROM cycle_scope_events WHERE id='${related}'; SELECT payload='{}'::jsonb AND reason IS NULL FROM admin_audit_logs WHERE id='${related}'; SELECT count(*) FROM domain_event_outbox WHERE id='${related}'; SELECT payload IS NULL FROM notifications WHERE id='${related}'; SELECT count(*) FROM mcp_proposals WHERE id='${competing}'; SELECT count(*) FROM mcp_proposals WHERE id='${competingUnscoped}'; SELECT issue_key,reason FROM cycle_scope_events WHERE id='${competing}'; SELECT payload FROM domain_event_outbox WHERE id='${competing}'; SELECT count(*) FROM external_refs WHERE id IN ('${commentRef}','${timeRef}') AND metadata IS NULL; SELECT count(*) FROM integration_sync_work WHERE id IN ('${commentWork}','${timeWork}') AND payload='{}'::jsonb; SELECT count(*) FROM integration_inbound_applications WHERE id IN ('${commentApplication}','${timeApplication}') AND outcome IS NULL; SELECT count(*) FROM integration_conflicts WHERE id IN ('${commentConflict}','${timeConflict}') AND local_evidence='{}'::jsonb AND remote_evidence='{}'::jsonb; SELECT metadata,payload FROM external_refs r JOIN integration_sync_work w ON w.ref_id=r.id WHERE r.id='${otherCommentRef}' AND w.id='${otherCommentWork}';`,
          ])
        ).stdout
          .trim()
          .split("\n")
