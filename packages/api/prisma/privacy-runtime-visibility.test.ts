...
const writeFenceUrl = new URL(
  "./migrations/20260826120000_privacy_write_dispatch_fencing/migration.sql",
  import.meta.url
);
const direct = [
  "issues:id",
...
describe("runtime privacy visibility migration", () => {
  ...
  describe.runIf(process.env.KAN246_RUN_DISPOSABLE_POSTGRES === "1")("PostgreSQL 16 proof", () => {
    it("hides held rows from runtime while preserving owner recovery visibility", async () => {
      ...
      await sql([
        "postgres",
        "INSERT INTO projects VALUES ('40404040-4040-4040-4040-404040404040','41414141-4141-4141-4141-414141414141'); INSERT INTO issues(id,key,title,privacy_held_at,privacy_hold_generation,project_id) VALUES ('42424242-4242-4242-4242-424242424242','KAN-OLD','held',clock_timestamp(),1,'40404040-4040-4040-4040-404040404040'); INSERT INTO integration_project_bindings VALUES ('43434343-4343-4343-4343-434343434343','40404040-4040-4040-4040-404040404040','active',NULL); INSERT INTO external_refs(id,entity_type,entity_id,binding_id,external_id) VALUES ('44444444-4444-4444-4444-444444444445','issue','42424242-4242-4242-4242-424242424242','43434343-4343-4343-4343-434343434343','old-remote'); INSERT INTO integration_sync_work(id,entity_type,entity_id,binding_id,payload) VALUES ('47474747-4747-4747-4747-474747474747','issue','42424242-4242-4242-4242-424242424242','43434343-4343-4343-4343-434343434343','{}'),('48484848-4848-4848-4848-484848484848','legacy','42424242-4242-4242-4242-424242424242','43434343-4343-4343-4343-434343434343','{}'); INSERT INTO integration_reconciliation_dispositions(id,preview_identity,remote_issue_id,remote_source_version,state,decision_kind,accepted_ref_id,binding_id,updated_at) VALUES ('45454545-4545-4545-4545-454545454545','46464646-4646-4646-4646-464646464646','old-remote','v1','linked','accept','44444444-4444-4444-4444-444444444445','43434343-4343-4343-4343-434343434343',clock_timestamp()); INSERT INTO privacy_authority.held_row_associations VALUES ('external_refs','44444444-4444-4444-4444-444444444445','42424242-4242-4242-4242-424242424242',1);",
      ]);
      ...
      await execFile("docker", ["cp", new URL(writeFenceUrl).pathname, `${container}:/tmp/write-fence.sql`]);
      await sql(["postgres", "INSERT INTO integration_sync_work(id,entity_type,entity_id,binding_id,payload,state) VALUES ('49494949-4949-4949-4949-494949494949','legacy',gen_random_uuid(),'43434343-4343-4343-4343-434343434343','{}','ambiguous')"]);
      await expect(execFile("docker", ["exec", container, "psql", "-v", "ON_ERROR_STOP=1", "postgresql://postgres:owner@localhost:5432/privacy", "-f", "/tmp/write-fence.sql"])).rejects.toThrow(/leased and ambiguous work to be drained/i);
      await sql(["postgres", "DELETE FROM integration_sync_work WHERE id='49494949-4949-4949-4949-494949494949'"]);
      await execFile("docker", ["exec", container, "psql", "-v", "ON_ERROR_STOP=1", "postgresql://postgres:owner@localhost:5432/privacy", "-f", "/tmp/write-fence.sql"]);
      expect((await sql(["postgres", "SELECT privacy_issue_id,privacy_hold_generation,state,skipped_reason,payload='{}'::jsonb,fence FROM integration_sync_work WHERE id='47474747-4747-4747-4747-474747474747'; SELECT count(*) FROM privacy_authority.held_row_associations WHERE store_kind='integration_sync_work' AND row_pk='47474747-4747-4747-4747-474747474747'; SELECT privacy_issue_id IS NULL AND privacy_hold_generation IS NULL FROM integration_sync_work WHERE id='48484848-4848-4848-4848-484848484848'"])).stdout.trim()).toBe("42424242-4242-4242-4242-424242424242|0|skipped|privacy_hold|t|1\n1\nt");
      expect((await sql(["kanon_runtime", "SELECT id FROM integration_sync_work WHERE id IN ('47474747-4747-4747-4747-474747474747','48484848-4848-4848-4848-484848484848')"])).stdout.trim()).toBe("48484848-4848-4848-4848-484848484848");
      ...
      expect(
        (
          await sql([
            "kanon_runtime",
            `SELECT id FROM comments; SELECT id FROM external_refs; SELECT id FROM integration_sync_work WHERE id IN ('${row}','${visible}'); SELECT id FROM domain_event_outbox; SELECT id FROM mcp_proposals;`,
          ])
        ).stdout.trim()
      ...
      const catalog = await sql([
        "postgres",
        "SELECT has_function_privilege('kanon_runtime','privacy_authority.issue_visible(uuid)','EXECUTE'), has_function_privilege('kanon_runtime','privacy_authority.row_visible(text,text)','EXECUTE'), has_function_privilege('kanon_runtime','privacy_authority.work_visible(text,uuid,integer,text,uuid,text,text)','EXECUTE'); SELECT count(*) FROM pg_policies WHERE schemaname='public' AND roles @> ARRAY['kanon_runtime']::name[] AND cmd='ALL'; SELECT count(*) FROM pg_class c JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND c.relrowsecurity AND NOT c.relforcerowsecurity; SELECT relrowsecurity FROM pg_class WHERE oid='public.domain_event_outbox'::regclass; SELECT count(*) FROM pg_trigger t JOIN pg_class c ON c.oid=t.tgrelid JOIN pg_namespace n ON n.oid=c.relnamespace WHERE n.nspname='public' AND t.tgname='privacy_runtime_preserve_hidden' AND NOT t.tgisinternal; SELECT relforcerowsecurity FROM pg_class WHERE oid='public.integration_sync_work'::regclass; SELECT p.prosecdef,pg_get_userbyid(p.proowner) FROM pg_proc p WHERE p.oid='privacy_authority.work_visible(text,uuid,integer,text,uuid,text,text)'::regprocedure; SELECT has_table_privilege('kanon_privacy_operator','public.integration_sync_work','UPDATE'),has_function_privilege('kanon_runtime','privacy_authority.transition_provider_io(uuid,text,integer,integer,boolean)','EXECUTE'),has_function_privilege('kanon_privacy_operator','privacy_authority.transition_provider_io(uuid,text,integer,integer,boolean)','EXECUTE'),has_column_privilege('kanon_runtime','public.integration_sync_work','provider_io_fence','UPDATE'),has_column_privilege('kanon_runtime','public.integration_sync_work','provider_io_fence','INSERT');",
      ]);
      ...
      const otherIssue = "30303030-3030-3030-3030-303030303030",
        ...
        projectWork = "35353535-3535-3535-3535-353535353535";
      await sql([
        "postgres",
      ...
      await sql(["postgres", `INSERT INTO integration_sync_work(id,entity_type,entity_id,binding_id,payload,state,skipped_reason,lease_token,lease_until,fence) VALUES (gen_random_uuid(),'issue','${issue}','${binding}','{"state":"retry"}','retry',NULL,'stale',clock_timestamp(),4),(gen_random_uuid(),'issue','${issue}','${binding}','{"state":"dead"}','dead',NULL,'stale',clock_timestamp(),4),(gen_random_uuid(),'issue','${issue}','${binding}','{"state":"ambiguous"}','ambiguous',NULL,'stale',clock_timestamp(),4),(gen_random_uuid(),'issue','${issue}','${binding}','{"state":"done"}','done',NULL,NULL,NULL,4),(gen_random_uuid(),'issue','${issue}','${binding}','{"state":"superseded"}','superseded',NULL,NULL,NULL,4),(gen_random_uuid(),'issue','${issue}','${binding}','{"state":"skipped"}','skipped','existing',NULL,NULL,4),('${projectWork}','project','${project}','${binding}','{}','queued',NULL,NULL,NULL,0);`]);
      expect((await sql(["postgres", `SELECT entity_type,privacy_issue_id,privacy_hold_generation FROM integration_sync_work WHERE id IN ('${related}','${commentWork}','${timeWork}') ORDER BY entity_type; SELECT privacy_issue_id IS NULL AND privacy_hold_generation IS NULL FROM integration_sync_work WHERE id='${projectWork}'`])).stdout.trim()).toBe(`comment|${issue}|0\nissue|${issue}|0\ntime_entry|${issue}|0\nt`);
      await expect(sql(["kanon_runtime", `INSERT INTO integration_sync_work(id,entity_type,entity_id,binding_id,payload,privacy_issue_id,privacy_hold_generation) VALUES (gen_random_uuid(),'issue','${issue}','${binding}','{}','${otherIssue}',99)`])).rejects.toThrow(/server managed/i);
      await expect(sql(["kanon_runtime", `UPDATE integration_sync_work SET privacy_hold_generation=99 WHERE id='${related}'`])).rejects.toThrow(/server managed/i);
      await expect(sql(["kanon_runtime", `INSERT INTO integration_sync_work(id,entity_type,entity_id,binding_id,payload) VALUES (gen_random_uuid(),'comment',gen_random_uuid(),'${binding}','{}')`])).rejects.toThrow(/privacy work root unavailable/i);
      ...
      expect((await sql(["postgres", `SELECT count(*) FILTER (WHERE state='skipped' AND skipped_reason='privacy_hold'),count(*) FILTER (WHERE state='done'),count(*) FILTER (WHERE state='superseded'),count(*) FILTER (WHERE state='skipped' AND skipped_reason='existing'),bool_and(payload='{}'::jsonb),bool_and(lease_token IS NULL AND lease_until IS NULL) FILTER (WHERE skipped_reason='privacy_hold'),count(*) FILTER (WHERE skipped_reason='privacy_hold' AND fence IN (1,5)) FROM integration_sync_work WHERE privacy_issue_id='${issue}'`])).stdout.trim()).toBe("6|1|1|1|t|t|6");
      expect(
        (
      ...
      const leasedIssue = "00000000-0000-0000-0000-000000000007",
        leasedBinding = "00000000-0000-0000-0000-000000000008",
        leasedRef = "00000000-0000-0000-0000-000000000009",
        leasedWork = "00000000-0000-0000-0000-00000000000a";
      await sql(["postgres", `INSERT INTO issues(id,key,title,description,project_id) VALUES ('${leasedIssue}','KAN-LEASE','Lease title','Lease body','${project}'); INSERT INTO integration_project_bindings VALUES ('${leasedBinding}','${project}','active',NULL); INSERT INTO external_refs(id,entity_type,entity_id,binding_id,external_id) VALUES ('${leasedRef}','issue','${leasedIssue}','${leasedBinding}','lease'); INSERT INTO integration_sync_work(id,entity_type,entity_id,binding_id,ref_id,payload,state,lease_token,lease_until,provider_io_fence) VALUES ('${leasedWork}','issue','${leasedIssue}','${leasedBinding}','${leasedRef}','{"secret":true}','leased','provider-started',clock_timestamp(),0),(gen_random_uuid(),'issue','${leasedIssue}','${leasedBinding}','${leasedRef}','{"history":true}','done',NULL,NULL,NULL);`]);
      await sql(["postgres", `UPDATE integration_sync_work SET state='ambiguous',lease_token=NULL,lease_until=NULL WHERE id='${leasedWork}'`]);
      expect((await sql(["kanon_runtime", `SELECT privacy_authority.transition_provider_io('${leasedWork}','stale',1,0,false)`])).stdout.trim()).toBe("0");
      await expect(sql(["kanon_runtime", `UPDATE integration_sync_work SET provider_io_fence=NULL WHERE id='${leasedWork}'`])).rejects.toThrow(/permission denied/i);
      await expect(sql(["kanon_privacy_operator", `BEGIN ISOLATION LEVEL SERIALIZABLE; SELECT privacy_authority.prepare_containment('00000000-0000-0000-0000-00000000000f','${leasedIssue}','${leasedBinding}'); SELECT privacy_authority.commit_containment('00000000-0000-0000-0000-00000000000f','${leasedIssue}','${leasedBinding}',1,'pq.gcm.v1:key:2:iv:cipher:tag'); COMMIT;`])).rejects.toThrow(/active provider I\/O/i);
      expect((await sql(["postgres", `SELECT title,description,privacy_held_at IS NULL,privacy_hold_generation FROM issues WHERE id='${leasedIssue}'; SELECT state,payload,provider_io_fence,fence FROM integration_sync_work WHERE id='${leasedWork}'; SELECT count(*) FROM privacy_quarantine.issue_content WHERE issue_id='${leasedIssue}'; SELECT count(*) FROM privacy_authority.held_row_associations WHERE issue_id='${leasedIssue}'`])).stdout.trim()).toBe("Lease title|Lease body|t|0\nambiguous|{\"secret\": true}|0|0\n0\n0");
      expect((await sql(["kanon_runtime", `SELECT privacy_authority.transition_provider_io('${leasedWork}','provider-started',0,0,false); UPDATE integration_sync_work SET state='retry' WHERE id='${leasedWork}'`])).stdout.trim()).toBe("1\nUPDATE 1");
      await sql(["kanon_privacy_operator", `BEGIN ISOLATION LEVEL SERIALIZABLE; SELECT privacy_authority.prepare_containment('00000000-0000-0000-0000-00000000000b','${leasedIssue}','${leasedBinding}'); SELECT privacy_authority.commit_containment('00000000-0000-0000-0000-00000000000b','${leasedIssue}','${leasedBinding}',1,'pq.gcm.v1:key:2:iv:cipher:tag'); COMMIT;`]);
      expect((await sql(["postgres", `SELECT state,skipped_reason,payload,fence,privacy_issue_id,privacy_hold_generation FROM integration_sync_work WHERE id='${leasedWork}'; SELECT state,payload FROM integration_sync_work WHERE privacy_issue_id='${leasedIssue}' ORDER BY state`])).stdout.trim()).toBe(`skipped|privacy_hold|{}|1|${leasedIssue}|0\ndone|{}\nskipped|{}`);
      await sql(["postgres", `UPDATE issues SET privacy_held_at=NULL,privacy_hold_generation=2 WHERE id='${leasedIssue}'; DELETE FROM privacy_authority.held_row_associations WHERE issue_id='${leasedIssue}'`]);
      expect((await sql(["kanon_runtime", `SELECT count(*) FROM integration_sync_work WHERE privacy_issue_id='${leasedIssue}'; UPDATE integration_sync_work SET state='queued' WHERE privacy_issue_id='${leasedIssue}'`])).stdout.trim()).toBe("0\nUPDATE 0");

      const deletedIssue = "00000000-0000-0000-0000-00000000000c",
        deletedRef = "00000000-0000-0000-0000-00000000000d",
        deleteWork = "00000000-0000-0000-0000-00000000000e";
      await sql(["postgres", `INSERT INTO issues(id,key,title,project_id) VALUES ('${deletedIssue}','KAN-DELETE','Delete title','${project}'); INSERT INTO external_refs(id,entity_type,entity_id,binding_id,external_id) VALUES ('${deletedRef}','issue','${deletedIssue}','${binding}','delete'); INSERT INTO integration_sync_work(id,entity_type,entity_id,binding_id,ref_id,payload,operation) VALUES ('${deleteWork}','issue','${deletedIssue}','${binding}','${deletedRef}','{}','delete'),(gen_random_uuid(),'issue','${deletedIssue}','${binding}','${deletedRef}','{}','update'); DELETE FROM issues WHERE id='${deletedIssue}';`]);
      expect((await sql(["kanon_runtime", `SELECT id,privacy_issue_id,privacy_hold_generation FROM integration_sync_work WHERE entity_id='${deletedIssue}'; UPDATE integration_sync_work SET state='retry' WHERE id='${deleteWork}'`])).stdout.trim()).toBe(`${deleteWork}|${deletedIssue}|0\nUPDATE 1`);
      ...
      await expect(
        sql([
          "kanon_runtime",
          `SET statement_timeout='200ms'; INSERT INTO integration_sync_work(id,entity_type,entity_id,binding_id,payload) VALUES (gen_random_uuid(),'issue','${raceIssue}','${raceBinding}','{}')`,
        ])
      ).rejects.toThrow(/statement timeout/);
      await racingContainment;
      await expect(sql(["kanon_runtime", `INSERT INTO integration_sync_work(id,entity_type,entity_id,binding_id,payload) VALUES (gen_random_uuid(),'issue','${raceIssue}','${raceBinding}','{}')`])).rejects.toThrow(/privacy work root unavailable/i);
      expect(
        (
          await sql([
            "postgres",
