...
| Step | Action |
|------|--------|
| 1. Download | Fetches `kanon-mcp-0.10.0.tar.gz` from the pinned GitHub Release |
...
```
https://raw.githubusercontent.com/mmmarxdr/kanon/mcp-v0.10.0/install.sh
```
...
> **Summary:** always use the tagged installer (`mcp-v0.10.0`). `main` will not
...
| Artifact | Naming |
|----------|--------|
| Tag | `mcp-v<version>` (e.g. `mcp-v0.10.0`) |
...
KANON_ONBOARD_LINK="kanon://your-link" bash -c "$(curl -fsSL https://raw.githubusercontent.com/mmmarxdr/kanon/mcp-v0.10.0/install.sh)"
...
1. Download `kanon-mcp-0.10.0.tar.gz` and `kanon-mcp-0.10.0.tar.gz.sha256` from the [GitHub Release](https://github.com/mmmarxdr/kanon/releases/tag/mcp-v0.10.0).
2. Verify: `sha256sum -c kanon-mcp-0.10.0.tar.gz.sha256` (Linux) or `shasum -a 256 -c kanon-mcp-0.10.0.tar.gz.sha256` (macOS).
3. Extract: `mkdir -p ~/.kanon/mcp && tar -xzf kanon-mcp-0.10.0.tar.gz -C ~/.kanon/mcp --strip-components=1`
