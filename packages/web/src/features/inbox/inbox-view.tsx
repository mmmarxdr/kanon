...
export function InboxView() {
  ...
  const greeting = greetingFor(user?.displayName ?? user?.email ?? "", t);
    ...
    <div style={{ display: "flex", height: "100%", overflow: "hidden" }}>
      ...
      >
        ...
        >
          ...
          >
            {t("subtitle", { agents: counts.activeAgents, issues: assigned.length })}
          </p>
        </div>
        ...
        >
          <Stat n={counts.openIssues} label={t("statOpenIssues")} />
          <Stat n={counts.inProgress} label={t("statInProgress")} tone="accent" />
          <Stat n={counts.awaitingReview} label={t("statAwaitingReview")} />
          <Stat n={counts.activeAgents} label={t("statActiveAgents")} tone="ai" />
        </div>

        {/* Assigned to you */}
        <Section title={t("sectionAssigned")} hint={isLoading ? t("loading") : undefined}>
          {assigned.length === 0 ? (
            <EmptyHint>{t("emptyAssigned")}</EmptyHint>
          ) : (
            assigned.map((issue) => (
              <InboxRow
                key={issue.key}
        ...
        {/* MCP proposals */}
        <Section
          title={t("sectionProposals")}
          hint={
            proposals.length > 0
              ? t("hintProposalsPending", { count: proposals.length })
              : t("hintProposalsEmpty")
          }
        >
          {proposals.length === 0 ? (
            <EmptyHint>
        ...
        >
          {(data?.mentions ?? []).length === 0 ? (
            <EmptyHint>{t("emptyMentions")}</EmptyHint>
          ) : (
            (data?.mentions ?? []).map((m) => (
        ...
        <Section
          ...
          hint={unreadCount > 0 ? t("hintUnread", { count: unreadCount }) : undefined}
          action={
            unreadCount > 0 ? (
              <button
                type="button"
        ...
        >
          {notificationsError ? (
            <EmptyHint data-testid="notifications-error">{t("errorNotifications")}</EmptyHint>
          ) : notificationList.length === 0 ? (
            <EmptyHint>{t("emptyNotifications")}</EmptyHint>
          ) : (
      ...
      >
        ...
        <RailCard title={t("railCurrentCycle")}>
          <CurrentCycleCard
        ...
        <RailCard title={t("railActiveAgents")} sub={t("railActiveAgentsSub")}>
          {agents.length === 0 ? (
            <EmptyHint>{t("emptyAgents")}</EmptyHint>
          ) : (
            <div
              style={{
                display: "flex",
        ...
        <RailCard title={t("railQuickActions")}>
          {/* Row 1: New issue — REQ-INBOX-QUICK-005 */}
          <QuickRow
            icon={<Icon.Plus />}
            label={t("actionNewIssue")}
            kbd="C"
          ...
          >
            {(open, disabled, isOpen) => (
              <QuickRow
                ...
                onClick={open}
                title={disabled ? t("noActiveProject") : undefined}
              />
            )}
          </ProjectPickerPopover>
  ...
  const key =
    hour < 12
      ...
      : hour < 18
        ? "greetingAfternoon"
        : "greetingEvening";
  const first = name.split(/[\s@]/)[0] ?? t("greetingFallback");
  const cap = first.charAt(0).toUpperCase() + first.slice(1);
  return t(key, { name: cap });
}
